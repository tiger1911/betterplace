...
public class DonationPageStepDefinitions extends DonationPage {
    ...
    public void the_user_is_on_the_donation_page() {
        String url = ConfigurationReader.get("url");
        Driver.get().get(url);
        Driver.get().findElement(By.xpath("//button[@class='btn btn-primary btn-large flex-grow mb-3']")).click();
    ...
    public void the_user_selects_radio_button(String radioButton) {
        ...
        sepaLastschriftRadioButton.click();
    }
    ...
    public void the_user_sends_copied_success_IBAN_value_to_the_inputBoxIBAN() {
       ...
       // BrowserUtils.scrollToElement(successIBANValue);
        String IBANValue = successIBANValue.getText();
        System.out.println(IBANValue);
        BrowserUtils.waitFor(2);

        BrowserUtils.scrollToElement(inputBoxIBAN);
        BrowserUtils.waitFor(2);
       // inputBoxIBAN.click();
        inputBoxIBAN.sendKeys(ConfigurationReader.get("validIBAN"));
